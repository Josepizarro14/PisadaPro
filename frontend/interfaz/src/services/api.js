...
const userApi = axios.create({
  baseURL: `${process.env.REACT_APP_USER_SERVICE_URL || 'http://localhost:5000'}/api`, // URL del microservicio de usuarios
  withCredentials: true,
});

// Instancia para el microservicio de productos
const productApi = axios.create({
  baseURL: process.env.REACT_APP_PRODUCT_SERVICE_URL || 'http://localhost:5001', // URL del microservicio de productos
  withCredentials: true,
});

// Instancia para el microservicio de productos
const catalogApi = axios.create({
  baseURL: process.env.REACT_APP_CATALOG_SERVICE_URL || 'http://localhost:5002', // URL del microservicio de catalogo
  withCredentials: true,
});

// Instancia para el microservicio de productos
const cartApi = axios.create({
  baseURL: `${process.env.REACT_APP_CART_SERVICE_URL || 'http://localhost:5003'}/cart`, // URL del microservicio de catalogo
  withCredentials: true,
});
